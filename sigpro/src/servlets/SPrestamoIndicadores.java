...
@WebServlet("/SPrestamoIndicadores")
public class SPrestamoIndicadores extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(accion.equals("getIndicadores")){
			...
			try{
				...
				response.setHeader("Content-Disposition", "attachment; Avance_de_Indicadores.xls");
				OutputStream outStream = response.getOutputStream();
				outStream.write(outArray);
				outStream.flush();
			}catch(Exception e){
				CLogger.write("1", SPrestamoIndicadores.class, e);
			}
		}else if(accion.equals("exportarPdf")){
//			CPdf archivo = new CPdf("Avance de Indicadores");
//			int proyectoId = Utils.String2Int(map.get("proyectoid"), 0);
//			int anioInicio = Utils.String2Int(map.get("fechaInicio"), 0);
//			int anioFin = Utils.String2Int(map.get("fechaFin"), 0);
//			int agrupacion = Utils.String2Int(map.get("agrupacion"), 0);
//			int tipoVisualizacion = Utils.String2Int(map.get("tipoVisualizacion"), 0);
//			String headers[][];
//			String datosIndicadores[][];
//			headers = generarHeaders(anioInicio, anioFin, agrupacion, tipoVisualizacion);
//			datosIndicadores = generarDatosMetas(proyectoId, anioInicio, anioFin, agrupacion, tipoVisualizacion, headers[0].length, usuario);
//			String path = archivo.ExportPdfMetasPrestamo(headers, datosIndicadores,tipoVisualizacion);
...
//			}
	...
	private stprestamo getIndicadores (ArrayList<ArrayList<BigDecimal>> indicadoresPrestamo,
			int anoInicial, int anoFinal, stprestamo prestamo){
		
		stanio[] anios = inicializarStanio(anoInicial, anoFinal);
		if(indicadoresPrestamo.size() > 0){
			
			for(ArrayList<BigDecimal> objprestamopresupuesto : indicadoresPrestamo){
	...
	private byte[] exportarExcel(int prestamoId, int anioInicio, int anioFin, int agrupacion, int tipoVisualizacion, String lineaBase, String usuario) throws IOException{
		...
		outArray = Base64.encode(outByteStream.toByteArray());
			...
			CLogger.write("4", SPrestamoIndicadores.class, e);
		}
		return outArray;
	}
