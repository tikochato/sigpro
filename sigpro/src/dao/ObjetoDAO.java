...
public class ObjetoDAO {
	...
	public static List<ObjetoCosto> getEstructuraConCosto(int idPrestamo, int anioInicial, int anioFinal, boolean obtenerPlanificado, boolean obtenerReal, boolean obtenerPresupuestos, String usuario) throws SQLException{
		...
		if(CMariaDB.connectAnalytic()){
			...
			List<?> estructuras = getConsultaEstructuraConCosto(idPrestamo);
				...
				if(obtenerPlanificado || obtenerPresupuestos){
					root = obtenerPlanificado(root, anioInicial, anioFinal, obtenerPresupuestos, conn_analytic);
				}		
	...
	public static List<ObjetoCostoJasper> getEstructuraConCostoJasper(Integer proyectoId, int anioInicial, int anioFinal, String usuario) throws SQLException{
		List<ObjetoCosto> listadoObjetos = getEstructuraConCosto(proyectoId, anioInicial, anioFinal, true, false, true, usuario);
	...
	private static ObjetoCosto obtenerPlanificado(ObjetoCosto objetoCosto, Integer anioInicial, Integer anioFinal, boolean obtenerPresupuestos, Connection conn){
		if(objetoCosto.getChildren()!=null && !objetoCosto.getChildren().isEmpty()){ //tiene hijos
			...
			for(int h=0; h<hijos.size(); h++){
				...
				hijo.anios = obtenerPlanificado(hijo, anioInicial, anioFinal, obtenerPresupuestos, conn).anios;
				for(int a=0; a<(anioFinal-anioInicial+1);a++){
					for (int m=0; m<12; m++){
