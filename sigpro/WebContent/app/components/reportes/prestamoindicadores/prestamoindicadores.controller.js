var app = angular.module('prestamoindicadoresController', [ 'smart-table']);


app.controller('prestamoindicadoresController',['$scope','$rootScope','$http','$interval','i18nService','Utilidades','$routeParams','$window','$location','$route','$mdDialog','$uibModal', '$document','$timeout','$q','$filter',
	...
	mi.iconoObjetoTipo = {
	    ...
	    mi.reiniciarVista=function(){
			if($location.path()=='/prestamoindicadores/rv')
				$route.reload();
			else
				$location.path('/prestamoindicadores/rv');
		}
	    
	$window.document.title = $utilidades.sistema_nombre+' - Avance de Indicadores';
	...
	$http.post('/SPrestamo', {accion: 'getPrestamos', t: (new Date()).getTime()}).then(
				...
				t:moment().unix()
					  ...
					  function successCallback(response) {
						    ...
						    anchor.attr({
						         ...
						         target: '_blank',
						         download: 'AvanceDeIndicadores.pdf'
						     })[0].click();
		...
		mi.cargarTabla = function() {			
			...
			$http.post('/SPrestamoIndicadores', datos)
				 ...
				 t:moment().unix()
					  ...
					  function successCallback(response) {
						  ...
						  anchor.attr({
					         ...
					         target: '_blank',
					         download: 'AvanceDeIndicadores.xls'
						  })[0].click();
					  }.bind(this), function errorCallback(response){
				 	}
			  	);
