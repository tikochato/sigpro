...
app.controller('formularioController',['$scope','$http','$interval','i18nService','Utilidades','$routeParams','$window','$location','$route','uiGridConstants','$mdDialog','$q','$uibModal', 'dialogoConfirmacion', 
	function($scope, $http, $interval,i18nService,$utilidades,$routeParams,$window,$location,$route,uiGridConstants,$mdDialog,$q,$uibModal, $dialogoConfirmacion) {
		...
		mi.gridOptions = {
			    ...
			    rowTemplate: '<div ng-dblclick="grid.appScope.formularioc.editarElemento($event)" ng-repeat="(colRenderIndex, col) in colContainer.renderedColumns track by col.uid" ui-grid-one-bind-id-grid="rowRenderIndex + \'-\' + col.uid + \'-cell\'" class="ui-grid-cell ng-scope ui-grid-disable-selection grid-align-right" ng-class="{ \'ui-grid-row-header-cell\': col.isRowHeader }" role="gridcell" ui-grid-cell="" ></div>',
				columnDefs : [
		...
		mi.borrar = function(ev) {
			if(mi.formularios!=null){
				$dialogoConfirmacion.abrirDialogoConfirmacion($scope
						...
						, '¿Desea borrar el Formulario "'+mi.formularios.nombre+'"?'
						, "Borrar"
						, "Cancelar")
