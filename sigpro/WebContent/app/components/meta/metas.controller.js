var app = angular.module('metasController', ['ui.grid.edit', 'ui.grid.rowEdit', 'ui.grid.pinning', 'ui.grid.autoResize']);

app.controller('metasController',['$scope','$http','$interval','i18nService','Utilidades','$routeParams','$window','$location','$route','uiGridTreeBaseService','uiGridConstants','$mdDialog', 'dialogoConfirmacion', 
		function($scope, $http, $interval,i18nService,$utilidades,$routeParams,$window,$location,$route,uiGridTreeBaseService,uiGridConstants,$mdDialog, $dialogoConfirmacion) {
			var mi=this;
			
			$window.document.title = $utilidades.sistema_nombre+' - Metas';
			i18nService.setCurrentLang('es');
			mi.mostrarcargando=true;
			//TODO: deberia ser false al inicio, por maquetacion lo mostramos
			...
			$http.post('/SMeta', { accion: 'getMetasUnidadesMedida', t: (new Date()).getTime() }).success(
					function(response) {
						...
						mi.gridOptions.columnDefs[3].editDropdownOptionsArray = mi.metaunidades;
			});
			
			$http.post('/SDatoTipo', { accion: 'cargarCombo', t: (new Date()).getTime() }).success(
					function(response) {
						mi.datoTipos = response.datoTipos;
						mi.gridOptions.columnDefs[4].editDropdownOptionsArray = mi.datoTipos;
			});
